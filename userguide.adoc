...
AsciiDoc是一种文本文档格式，用于编写笔记，文档，文章，书籍，电子书，幻灯片，网页，博客和UNIX man页面。AsciiDoc文件可以翻译成多种格式，包括HTML，PDF，EPUB，手册页。AsciiDoc是高度可配置的：AsciiDoc源文件语法和后端输出标记（可以是几乎任何类型的SGML / XML标记）都可以由用户自定义和扩展。

.本文档
**********************************************************************
本文档非常庞大，因此可能需要重新设计，拆分成教程、快捷参考和正式参考手册。

如果您不熟悉AsciiDoc，请阅读本节和 <<X6,入门>> 部分，并查看分发 `doc` 目录中的示例AsciiDoc (`*.txt`)源文件。
**********************************************************************


介绍
--
AsciiDoc是一种纯文本的人类可读/可写文档格式，可以使用asciidoc命令转换为DocBook或HTML。
然后，您可以直接使用asciidoc生成的HTML。
也可以通过您喜欢的DocBook工具链运行asciidoc DocBook输出。
或者使用AsciiDoc a2x工具链包装器生成PDF，EPUB，DVI，LaTeX，PostScript，手册页，HTML和文本格式。

AsciiDoc格式本身就是一种有用的演示格式：AsciiDoc标记简单，直观，因此易于校对和编辑。

AsciiDoc重量轻：它包含一个Python脚本和一堆配置文件。 
除了asciidoc和Python解释器，不需要其他程序即可将AsciiDoc文本文件转换为DocBook或HTML。 
请参阅下面的 <<X11,AsciiDoc文档示例>> 。

文本标记约定通常是（通常是很强的）个人喜好问题：如果默认语法不符合您的喜好，则可以通过编辑基于文本的asciidoc配置文件来定义自己的语法。
您还可以创建配置文件以将AsciiDoc文档转换为几乎所有SGML / XML标记。

asciidoc提供了一组配置文件，可将AsciiDoc的文章，书籍和手册页转换为HTML或DocBook后端格式。

.我的AsciiDoc动机
**********************************************************************
DocBook已经成为事实上的标准开放源代码文档格式。但是DocBook是一种复杂的语言，标记难以阅读，甚至更难直接编写 -- 与编写文档相比，我发现我花更多的时间输入标记标签，查阅参考手册和修复语法错误。
**********************************************************************
...
安装AsciiDoc
~~~~~~~~~~
在安装之前，请阅读完`README`和`INSTALL`文件，打包人员还要查看一下 <<X38,打包笔记>>。

[[X11]]
AsciiDoc文档示例
~~~~~~~~~~~~
快速了解AsciiDoc的最佳方法是查看AsciiDoc网站或者已经发布的示例：

- 看一下AsciiDoc网站主页 {网站} 上的链接示例。 
  通过点击侧边栏的 `Page Source` 菜单项来查阅对应的AsciiDoc源代码。
- 阅读分发版本`./doc`目录中的`*.txt`源文件以及相应的HTML和DocBook XML文件。


AsciiDoc文档类型
------------
AsciiDoc有三种文档类型:文章,书籍和帮助页面。
所有文档类型共用相同的AsciiDoc格式，但有一些细微变化。
如果您熟悉DocBook，您会注意到AsciiDoc文档类型与同名的DocBook文档类型相对应。

asciidoc(1) 命令使用 `-d` (`--doctype`) 选项指定AsciiDoc文档类型 -- 默认的类型是'文章'。

对于AsciiDoc文档源文件而言，改变 `.txt`文件拓展名是有效的。

文章
~~
用于简短文档，文章和常规文档。 请参阅AsciiDoc发行版 `./doc/article.txt` 示例。

AsciiDoc定义了标准的DocBook文章的题前和题尾<<X93,节标记模板>>（附录，摘要，参考书目，词汇表，索引）。

书籍
~~
书籍与文章共享相同的格式，但内容有以下差异：

- 在多部分书籍中，每一部分的标题是一个 <<X17,顶层标题>> (和书籍标题同等级)。
- 有一些部分是书籍特有的，例如序言和版权页。

由于DocBook处理器可以自动生成脚注，目录，表列表，图形列表，示例列表和索引，
因此通常会使用书籍文档来生成DocBook输出。

.书籍文档示例
书籍::
  AsciiDoc发行版本中的 `./doc/book.txt` 文件。

多部分书籍::
  AsciiDoc发行版本中的  `./doc/book-multi.txt` 文件。

帮助页面
~~~~
用于生成roff格式的UNIX手册页。AsciiDoc帮助文档遵守特殊的标题标题和节命名约定。  AsciiDoc manpage
-- 详情请查看 <<X1,帮助文档>> 部分.

AsciiDoc通过定义 `摘要` 来生成 DocBook文档的 `refsynopsisdiv` 部分。

也可以查看AsciiDoc发布版本中的帮助手册源文件(`./doc/asciidoc.1.txt`)。


[[X5]]
AsciiDoc后端
---------
asciidoc命令会将AsciiDoc格式文件转换为由 `-b`(`--backend`)选项参数指定的后端格式。
asciidoc(1)本身几乎不了解后端格式，所有转换规则都包含在可自定义的级联配置文件中。
后端特定属性在 <<X88,后端属性>>部分中列出。
...
xhtml11::
  该后端生成使用CSS2样式的XHTML 1.1标记。 输出文件具有`.html`扩展名。

html5::
  该后端生成HTML5标记，除了包含音频和视频块宏之外，它在功能上与“ xhtml11”后端相同。

slidy::
  使用此后端可以从AsciiDoc文档为Web浏览器生成独立的
  http://www.w3.org/Talks/Tools/Slidy2/[Slidy] HTML幻灯片。
  Slidy后端的相关文档在发行版本的`doc/slidy.txt`文件中或者 {website}slidy.html[此处网页]。

wordpress::
  “ html4”后端的一个较小变体，用于支持 http://srackham.wordpress.com/blogpost1/[博客文章]。
...
后端别名
~~~~
后端别名是AsciiDoc后端的备用名称。
AsciiDoc带有两个后端别名:'html'('xhtml11'的别名)和
'docbook'('docbook45'的别名).

您可以通过将名为`backend-alias- <别名>`的AsciiDoc属性设置为AsciiDoc后端名称来分配（或重新分配）后端别名。
例如，下面的这个后端别名属性定义包含在全局`asciidoc.conf`文件的 `[attributes]` 部分中。
配置文件如下：
...
后端插件
~~~~
asciidoc(1) 命令的 `--backend`选项也用来安装和管理后端 <<X101,插件>>。

- 后端插件的用法与内置后端一样。
- 后端插件优先于具有相同名称的内置后端。
- 您可以使用{asciidoc-confdir} <<X60,内部属性>> 从后端插件配置文件引用内置的后端配置文件位置。
- 您可以使用{backend-confdir} <<X60,内部属性>> 来引用后端插件配置文件的位置。
- 后端插件默认安装在`$HOME/.asciidoc/backends/<backend>` 中。
  其中 `<backend>` 是后端的名称。


DocBook
-------
AsciiDoc生成'文章'，'书籍'和'文档'类型的DocBook文档（与AsciiDoc'文章'，'书籍'和'手册页'文档类型相对应）。

大多数Linux发行版都带有转换工具（统称为工具链），用于将 <<X12,DocBook文件转换>> 为演示文稿格式，
例如Postscript，HTML，PDF，EPUB，DVI，PostScript，LaTeX，roff（本机手册页格式），HTMLHelp，JavaHelp和文本 。
还有一些程序可让您直接查看DocBook文件，例如 http://live.gnome.org/Yelp[Yelp]（GNOME帮助查看器）。

[[X12]]
将DocBook转换为其他文件格式
~~~~~~~~~~~~~~~~~
使用称为DocBook'工具链'的应用程序组合来验证、解析和翻译DocBook文件的各种演示文稿文件格式。
这个工具链的一个作用就是读取DocBook标记(由AsciiDoc产生)，并把它们转换成一种演示文档格式(例如：
HTML、PDF、HTML Help、EPUB、DVI，PostScript或者LaTex)。

各种各样的用户输出格式要求以及可用工具和样式表的选择导致了许多有效的工具链组合。

[[X43]]
a2x工具链包装
~~~~~~~~
新用户面临的最大障碍之一是安装，配置和使用DocBook XML工具链。
我们可以通过 `a2x(1)` 命令来获取一些帮助 -- 这是一个工具链包装程序命令。
该命令会生成XHTML（分块和非分块），PDF，EPUB，DVI，PS，LaTeX，手册页，HTML帮助以及来自AsciiDoc文本文件的文本文件输出。
a2x(1)会完成所有与生成和排序工具链命令以及管理中间文件和输出文件相关的繁琐工作。
`a2x(1)` 还可以选择性的部署警告和导航图标以及CSS样式表。
详细说明请查看 `a2x(1)` 的帮助手册。
除了asciidoc(1)，您还需要xsltproc(1)，DocBook XSL样式表以及可选的:dblatex或FOP（生成PDF）; w3m(1)或lynx(1)（生成文本）。

以下示例从AsciiDoc `doc/source-highlight-filter.txt` 源文件生成 `doc/source-highlight-filter.pdf` 文件。
第一个示例使用dblatex(1)（默认的PDF生成器），第二个示例强制使用FOP：
...
TIP: 使用--verbose命令行选项查看执行的工具链命令。

HTML生成
~~~~~~
AsciiDoc无需DocBook工具链即可直接生成样式精美的HTML，但是走DocBook路线也有很多优势:

- DocBook的HTML可以选择包括自动生成的索引，目录，脚注，图形和表格列表。
- DocBook工具链还可以（可选）为每个文档部分生成单独的（分块）链接的HTML页面。
- 工具链处理执行链接和文档有效性检查。
- 如果设置了DocBook 'lang' 属性，
  则将以指定的语言输出目录，图形标题和表格标题以及警告标题之类的东西
  （设置AsciiDoc'lang'属性将设置DocBook 'lang' 属性）。

另一方面，直接从AsciiDoc输出的HTML速度更快，易于定制，
并且可以在没有合适的DocBook工具链的情况下使用(示例请查看 {website}[AsciiDoc官网]).

PDF生成
~~~~~
有两种常用的从DocBook生成PDF的工具，
<<X31,dblatex>> 和 <<X14,FOP>>。

.dblatex 还是 FOP?
- 'dblatex' 很容易安装, 它不需要做额外的配置，并且也不需要安装JAVA虚拟机 -- 开箱即用。
- 'dblatex' 源代码的高亮和行号显示非常出色。
- “ dblatex”更易于使用，因为它将DocBook直接转换为PDF，而在使用“ FOP”之前，您必须使用 <<X13,DocBook XSL样式表>> 将DocBook转换为XML-FO。
- 'FOP' 功能更完整（例如，标注在文字布局内处理），可以产生更好看的输出。
...
文档结构
----
AsciiDoc文档由一系列块元素组成，这些块元素以可选的文档标题开头，然后是可选的前言(Preamble)，然后是零个或多个文档节。

几乎零个或多个元素的任何组合都构成一个有效的AsciiDoc文档：
文档的范围可以从单个句子到多部分的书。

块级元素
~~~~
块元素包含一行或多行文本，并且可以包含其他块元素。

AsciiDoc块结构可以非正式地总结如下
footnote:[这是一个粗略的结构指南，而不是严格的语法定义]:
...
- '?' 表示出现零次或者一次, '+' 表示出现一次或者多次, '*' 表示出现零次或者多次
- 所有块元素都由线边界分隔。
- `BlockId`, `AttributeEntry` and `AttributeList` 
  这些块元素(不会显示出来) 可以出现在任何地方。
- 对块语法有许多文档类型和特定于后端的限制。
- 以下元素不能包含空行: Header, Title,
  Paragraph, ItemText.
- ListParagraph是具有 'listelement' 选项集的段落。
- ListContinuation 是一个 <<X15,列表后续元素>>.

[[X95]]
标题头部
~~~~
标头包含文档元数据，通常是标题以及可选的作者和修订信息:

- 标头是可选的，但是如果使用标头，则必须以文档 <<X17,标题>> 开头。
- 标头标题后面紧跟着可选的作者和修订信息。
- 文档标题必须与文档的其余部分用一个或多个空白行分隔，并且不能包含空白行。
- 标头可以包含注释。
- 表头可以包含 <<X18,属性条目>>, 典型的有
  'doctype', 'lang', 'encoding', 'icons', 'data-uri', 'toc',
  'numbered'。
- 标头属性被命令行属性覆盖。
- 如果标头包含非UTF-8字符，则 'encoding' 必须在标头之前（在文档中或在命令行中）。
...
作者信息行包含作者的姓名，并可选地后面带有作者的电子邮件地址。 
作者的姓名格式如下:

  firstname[ [middlename ]lastname][ <email>]]

即名字，然后是可选的中间名和姓氏，然后是该电子邮件地址。
可以使用下划线作为单词分隔符来输入多单词的名字，中间名和姓氏。
电子邮件地址位于最后，必须用尖括号<>括起来。
这里是一些作者信息行的例子:
...
如果作者行与上述规范不匹配，则将整个作者行视为名字。

可选的修订信息行位于作者信息行之后。
修订信息可以是以下两种格式之一：

. 可选文档修订号(版本号)，后跟可选修订日期，后跟可选修订说明：
+
--
  * 如果指定了修订号，则必须在其后加上逗号。
  * 版本号必须至少包含一个数字字符。
  * 第一个数字字符之前的所有非数字字符都将被删除。
  * 如果指定了修订说明，则必须在前面加上冒号。
    修订说明从冒号一直延伸到下一个空白行，属性条目或注释，并且可以使用常规文本替换。
  * 如果设置了修订号或备注，但尚未设置修订日期，
    则将修订日期设置为 'docdate' 属性的值。
...
. 修订信息行还可以是RCS/CVS/SVN $Id$标记：
+
--
  * AsciiDoc从$Id$修订标记中提取 'revnumber'，'revdate' 和 'author' 属性，
    并将其显示在文档标题中。
  * 如果使用$Id$修订标记，则可以省略标题作者行。
...
您可以通过使用asciidoc（1）`-a`（`--attribute`）命令行选项传递'revnumber'，'revremark'，'revdate'，'email'，'author'，'authorinitials'，'firstname' 和 'lastname' 属性来覆盖或设置标头参数。 
例如:

  $ asciidoc -a revdate=2004/07/27 article.txt

还可以将属性条目添加到标题中，以便在标题模板中用 <<X18,Attribute Entry>> 元素替换。

HTML输出中的 'title' 元素设置为AsciiDoc文档标题，您可以通过在文档标题中包含 'title' 属性条目将其设置为其他值。

[[X87]]
其他文件标题信息
^^^^^^^^
AsciiDoc具有两种机制来实现可选择性在输出文档的标题中包含其他元数据:
...
标题
--
文档标题和节标题可以采用以下两种格式之一：

双行标题
~~~~
双行标题包含一个左边顶格开始的标题行和一个下划线行。
下划线部分由重复的字符组成，这些字符对应其上的标题的宽度（相等或最多多两个字符）:
...
单行标题
~~~~
一行标题由单行组成，该行在任一侧由一个或多个等号字符定界（等号字符的数量对应于节级别减去一个）
下面是一些例子:
...
- 标题和定界符之间必须有一个或多个空格。
- 标题结尾定界符是可选的。
- 可以通过编辑配置文件 `[titles]` 小节 `sect0` ... `sect4` 条目来更改单行标题语法。
=====================================================================

浮动标题
~~~~
将标题的第一个位置属性或 `样式` 属性设置为 `float` 会生成一个自由浮动的标题。
自由浮动标题的呈现方式与普通节标题一样，但未与文本主体正式关联，也不属于常规节层次结构，因此常规排序规则不适用。
浮动标题也可用于节标题不合法的情况下：例如边栏和告诫栏。例如：
...
块标题
---
'BlockTitle' 元素是单行，以点标点(.)开头，后跟标题文本。
BlockTitle应用于紧随其后的Paragraph，DelimitedBlock，List，Table或BlockMacro。
例如:
...
块id元素
-----
'BlockId' 是一个单行块元素，其中包含一个用双方括号括起来的唯一标识符。 用于将标识符分配给随后的块元素。
例如：

  [[chapter-titles]]
  Chapter titles can be ...

上面的示例标识了随后的段落，因此可以从其他位置引用该段落, 就像这样
`<<chapter-titles,chapter titles>>`.

'BlockId' 元素可以应用于Title，Paragraph，List，DelimitedBlock，Table和BlockMacro元素。
BlockId元素设置 `{id}` 属性以在后续块的标记模板中进行替换。
如果提供了第二个位置参数，它将设置 `{reftext}` 属性，该属性用于设置DocBook的 `xreflabel` 属性。

'BlockId' 元素与 <<X30,锚定内联宏>> 具有相同的语法和相同的功能。
